...
class Stock:
    ...
    def getAnnualFinancialsReported(self):
        r = requests.get('https://finnhub.io/api/v1/stock/financials-reported?symbol='+tickerName+'&token='+API_KEY)
        return r.json()
    
    def getCompanyProfile(self):
        r = requests.get('https://finnhub.io/api/v1/stock/profile2?symbol=AAPL&token=brjcsenrh5r9g3ot1d8g')
        return r.json()
    ...
    def getPositionValue(cls, positionsJSON):
        ...
        for positionKey in positionsJSON.keys():
            if (positionKey == "name"):
                continue
            value += yahoo_fin.stock_info.get_live_price(positionKey) * positionsJSON[positionKey]
        return value
