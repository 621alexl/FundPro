...
def sell(request):
    ...
    positionsDict = (json.loads(str(CurrentTrader.positions).replace("'", '"')))
    CurrentTrader.AUM = Stock.getPositionValue(positionsDict) + float(CurrentTrader.cash)
    if request.method == "POST":
        ...
        if (ShareCount > positionsDict[TickerName]):
            messages.add_message(request, messages.INFO, 'Too many stocks being sold')
        ...
        if (TickerName in positionsDict.keys()):
            ...
            messages.add_message(request, messages.INFO, 'Do not own this stock')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if graphTime is not None:
                ...
                plt=stock.getPlotlyPriceHistory(titleName, price, period="1d", interval="1m")
